...
@app.route("/")
def index():
    link = "<h1>歡迎進入陳凱甯的網站首頁</h1>"
    link += "<a href=/mis>課程</a><hr>"
    link += "<a href=/today>今天日期</a><hr>"
    link += "<a href=/about>關於凱甯</a><hr>"
    link += "<a href=/welcome?u=凱甯&dep=靜宜資管>GET傳值</a><hr>"   
...
def webhook3():
    ...
    if action == "rateChoice":
        ...
        info = "我是陳凱甯開發的電影聊天機器人，您選擇的電影分級是：" + rate + "，相關電影：\n"
        db = firestore.client()
...
def weather():
    ...
    if request.method == "POST":
        ...
        Data = requests.get(url, verify=False)  # :point_right: 加上 verify=False 以防止 SSL 驗證錯誤
